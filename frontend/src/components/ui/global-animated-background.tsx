...
export const GlobalAnimatedBackground = () => {
  return (
    <div className="fixed inset-0 z-[-1] w-full h-full bg-black pointer-events-none overflow-hidden">
      <div className="absolute inset-0">
        {/* నువ్వు పంపిన ఎగ్జాక్ట్ వాల్యూస్ ఇక్కడే ఉన్నాయి */}
        <CanvasRevealEffect
          animationSpeed={3}
          colors={[
            [255, 255, 255],
      ...
      {/* ఒరిజినల్ కోడ్ లో ఉన్న ఎగ్జాక్ట్ గ్రేడియంట్ ఓవర్లేస్ ఇవే */}
...
// --- 2. Canvas & Shader Components (Exact from your code) ---
...
const ShaderMaterial = ({ source, uniforms }: any) => {
  ...
  const material = useMemo(() => new THREE.ShaderMaterial({
      ...
      void main(){ gl_Position = vec4(position.x, position.y, 0.0, 1.0); fragCoord = (position.xy + vec2(1.0)) * 0.5 * u_resolution; fragCoord.y = u_resolution.y - fragCoord.y; }
      `,
...
const Shader: React.FC<{ source: string; uniforms: any; maxFps?: number }> = ({ source, uniforms }) => (
  <Canvas className="absolute inset-0 h-full w-full"><ShaderMaterial source={source} uniforms={uniforms} /></Canvas>
);